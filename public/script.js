document.addEventListener('DOMContentLoaded', () =>{
    ...
    function startMultiPlayer(){
        ...
        socket.on('player-connection', num =>{
            console.log(`Player number ${num} has C or D`)
        ...
        //On timout
        socket.on('timeout', () =>{
            infoDisplay.innerHTML = 'You Have reached the 3 minute limit';
        })

        //Ready button click
        startButton.addEventListener('click', ()=>{
            if(allShipsPlaced) playGameMulti(socket);
            else infoDisplay.innerHTML = "Please Plsce all Ships";
        })
    ...
    function generateShip(ship){
        ...
        var isTaken = current.some(index => computerSquares[randomStart + index].classList.contains('taken'));
        var isAtRightEdge = current.some(index => (randomStart + index) % width === width - 1);
        var isAtLeftEdge = current.some(index => (randomStart + index) % width === 0);
    ...
    function rotate(){
        if(isHorizontal){
            ...
            carrier.classList.toggle('carrrier-container-vertical');
            isHorizontal = false
        }
        ...
        if(!isHorizontal){
            ...
            carrier.classList.toggle('carrrier-container');
            isHorizontal = true
        }
    }
    ...
    function checkForWins(){
        ...
        if(cpuDestroyerCount === 2){
            infoDisplay.innerHTML = `You lost your Destroyer...`;
            cpuDestroyerCount = 10;
        }
        if(cpuSubmarineCount === 3){
            infoDisplay.innerHTML = `You lost your Submarine...`;
            cpuSubmarineCount = 10;
        }
        if(cpuCruiserCount === 3){
            infoDisplay.innerHTML = `You lost your Cruiser...`;
            cpuCruiserCount = 10;
        }
        if(cpuBattleshipCount === 4){
            infoDisplay.innerHTML = `You lost your Battleship...`;
            cpuBattleshipCount = 10;
        }
        if(cpuCarrierCount === 5){
            infoDisplay.innerHTML = `You lost your Carrier...`;
            cpuCruiserCount = 10;
        }
